...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle)
{
    ...
    // infinite while loop to block code exection until intersection light is green 
    while(true)
    {
        if(_trafficLight.waitForGreen() && _trafficLight.getCurrentPhase() == TrafficLightPhase::green)
            break;
    }
...
void Intersection::simulate() // using threads + promises/futures + exceptions
{
    ...
    // launch TrafficLight simulation in a thread 
